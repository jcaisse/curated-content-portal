...
function validateComposeVars() {
  console.log('🔍 Validating Docker Compose variable expansions...');
  ...
  for (const fileName of composeFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Check for unknown variable expansions
      const unknownVarPattern = /\$\{[A-Z_]+\}/g;
      const matches = line.match(unknownVarPattern);
      if (matches) {
        for (const match of matches) {
          const varName = match.slice(2, -1);
          // Allow common Docker Compose variables and our known safe ones
          const allowedVars = ['APP_PORT', 'POSTGRES_USER', 'POSTGRES_DB', 'NODE_ENV'];
          if (!allowedVars.includes(varName) && !varName.includes(':-')) {
            console.error(`❌ ${fileName}:${lineNum}: Unknown variable expansion: ${match}`);
            console.error(`   Line: ${line.trim()}`);
            console.error(`   💡 Use env_file: or whitelist this variable`);
            hasErrors = true;
          }
        }
      }
